...
#undef get_g
    ...
    void SearchInstance::set_end_polygon(const PointLocation& end_pl)
    {
        // Any polygon is fine.
...
#undef get_lazy
    ...
    void SearchInstance::gen_initial_nodes(const PointLocation& start_pl)
    {
        // {parent, root, left, right, next_polygon, right_vertex, f, g}
        // be VERY lazy and abuse how our function expands collinear search nodes
        // if right_vertex is not valid, it will generate EVERYTHING
...
#define root_to_point(root) ((root) == -1 ? start : mesh->mesh_vertices[root].p)
    ...
    bool SearchInstance::search_covisible(PointLocation start_pl, PointLocation end_pl){
    ...
    bool SearchInstance::triangle_search(vector<tuple<int,int>>& cur_triangle)
    {
        timer.start();
        this->init_search();
        ...
        while (!open_list.empty())
        {
            ...
            cur_triangle.push_back(make_tuple(node->left_vertex,node->right_vertex));
...
continue;
    ...
    bool SearchInstance::search(PointLocation start_pl ,PointLocation end_pl)
    {
        timer.start();
        init_search(start_pl,end_pl);
    ...
    bool SearchInstance::bounded_search(PointLocation start_pl ,PointLocation end_pl,double bound)
    {
        timer.start();
        init_search(start_pl,end_pl);
...
//            std::cout<<std::endl;
    ...
    void SearchInstance::get_path_vertex_id(std::vector<int>& out,int s_id,int g_id)
    {
        if (final_node == nullptr)
        {
            return;
