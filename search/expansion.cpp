...
// Generates the successors of the search node and sets them in the successor
// vector. Returns number of successors generated.
    int get_grid_observable_successors(SearchNode& node, const Point& start, const Mesh& mesh,
                       Successor* successors)
    {
...
//    Orientation  a  = get_orientation(root, node.left, node.right);
        ...
        //TBR!Changed to if its CCW then it shouldn't generate any observable successors
        //assert(get_orientation(root, node.left, node.right) !=
        //       Orientation::CCW);
        if(get_orientation(root,node.left,node.right)!= Orientation::CCW){
            return 0;
        }
...
#define index2point(index) mesh_vertices[V[index]].p
        ...
        //TBR!Cast everything into if statement
        //assert(A != -1);
        if(A == -1){
            const int normalised_A = normalise(A),
            normalised_Am1 = normalise(A-1);
...
#undef normalise
        ...
        //TBR! Change assert to if statement
        //assert(B != -1);
